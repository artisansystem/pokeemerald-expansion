...
void SpawnLinkPartnerObjectEvent(void)
{
    ...
    for (i = 0; i < gSpecialVar_0x8004; i++)
    {
        if (myLinkPlayerNumber != i)
        {
            ...
            default:
            {
                u8 outfit = gLinkPlayers[i].currOutfitId, gender = gLinkPlayers[i].gender, appearance = gLinkPlayers[i].appearance;
                if (outfit < OUTFIT_COUNT)
                    linkSpriteId = GetLinkPlayerAvatarGraphicsIdByStateIdLinkIdAndGender(PLAYER_AVATAR_STATE_NORMAL, i, gender, appearance);
                else
...
static void LoadLinkPartnerObjectEventSpritePalette(u16 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    while (i < MAX_LINK_PLAYERS)
    {
        ...
        appearance = gLinkPlayers[i].appearance;
        outfit = gLinkPlayers[i].currOutfitId;
        gfx = GetPlayerAvatarGraphicsIdByOutfitStateIdAndGender(outfit, PLAYER_AVATAR_STATE_NORMAL, gender, appearance);
...
void SetBattleTowerLinkPlayerGfx(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (gLinkPlayers[i].gender == MALE)
            VarSet(VAR_OBJ_GFX_ID_F - i, PLAYER_AVATAR_GFX_LIGHT_MALE_NORMAL);
        else
            VarSet(VAR_OBJ_GFX_ID_F - i, PLAYER_AVATAR_GFX_LIGHT_FEMALE_NORMAL);
    }
}
