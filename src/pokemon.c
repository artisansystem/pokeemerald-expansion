...
#define DAWNSINGER_TO_NATIONAL(name) [DAWNSINGER_DEX_##name - 1] = NATIONAL_DEX_##name,
#define UNDERGROUND_TO_NATIONAL(name) [UNDERGROUND_DEX_##name - 1] = NATIONAL_DEX_##name,
...
static const enum NationalDexOrder sDawnsingerToNationalOrder[DAWNSINGER_DEX_COUNT - 1] =
{
    FOREACH_SPECIES_IN_DAWNSINGER_DEX_ORDER(DAWNSINGER_TO_NATIONAL)

};

static const enum NationalDexOrder sUndergroundToNationalOrder[UNDERGROUND_DEX_COUNT - 1] =
{
    FOREACH_SPECIES_IN_UNDERGROUND_DEX_ORDER(UNDERGROUND_TO_NATIONAL)
};
...
void CreateMon_Legacy(struct Pokemon *mon, enum Species species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    u32 mail;
    ZeroMonData(mon);
    CreateBoxMon_Legacy(&mon->box, species, level, fixedIV, hasFixedPersonality, fixedPersonality, otIdType, fixedOtId);
    SetMonData(mon, MON_DATA_LEVEL, &level);
    mail = MAIL_NONE;
    SetMonData(mon, MON_DATA_MAIL, &mail);
...
enum DawnsingerDexOrder NationalToDawnsingerOrder(enum NationalDexOrder nationalNum)
{
    ...
    while (dawnsingerNum < (DAWNSINGER_DEX_COUNT - 1) && sDawnsingerToNationalOrder[dawnsingerNum] != nationalNum)
        dawnsingerNum++;

    if (dawnsingerNum >= DAWNSINGER_DEX_COUNT - 1)
        return 0;

    return dawnsingerNum + 1;
...
{
    if (!dawnsingerNum || dawnsingerNum >= DAWNSINGER_DEX_COUNT)
        return 0;

    return sDawnsingerToNationalOrder[dawnsingerNum - 1];
}

enum NationalDexOrder UndergroundToNationalOrder(enum UndergroundDexOrder undergroundNum)
{
    if (!undergroundNum || undergroundNum >= UNDERGROUND_DEX_COUNT)
        return 0;

    return sUndergroundToNationalOrder[undergroundNum - 1];
}
...
enum TrainerPicID PlayerGenderToFrontTrainerPicId(enum Gender playerGender)
{
    if (playerGender == MALE)
        ...
        return FacilityClassToPicIndex(FACILITY_CLASS_WALLY);
}
