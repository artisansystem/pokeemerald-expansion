...
static void FindMapsWithMon(enum Species species)
{
    ...
    for (i = 0; gWildMonHeaders[i].mapGroup != MAP_GROUP(MAP_UNDEFINED); i++)
    {
        ...
        if (MapHasSpecies(&gWildMonHeaders[i].encounterTypes[gAreaSeason][gAreaTimeOfDay], headerSectionId, species))
        {
...
void DisplayPokedexAreaScreen(enum Species species, u8 *screenSwitchState, enum Seasons season, enum TimeOfDay timeOfDay, enum PokedexAreaScreenState areaState)
{
    u8 taskId;
