...
static const u16 sPokedexPlusHGSS_Underground_Pal[] = INCGFX_U16("graphics/pokedex/hgss/palette_underground.pal", ".gbapal");
...
static const u8 sPokedexModes[] = {DEX_MODE_DAWNSINGER, DEX_MODE_UNDERGROUND, DEX_MODE_NATIONAL};
...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
    ...
    pokedexView->dexMode = DEX_MODE_DAWNSINGER;
    pokedexView->dexModeBackup = DEX_MODE_DAWNSINGER;
...
#define temp_dexNum     vars[3]
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < NATIONAL_DEX_COUNT; i++)
        {
            ...
            if ((!temp_isDawnsingerDex || NationalToDawnsingerOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = NATIONAL_DEX_COUNT - 1; i >= 0; i--)
        {
            ...
            if ((!temp_isDawnsingerDex || NationalToDawnsingerOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
static u8 LoadInfoScreen(struct PokedexListItem *item, u8 monSpriteId)
{
    ...
    gAreaSeason = CurrentSeasonGetForDex();
...
static void Task_LoadAreaScreen(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        DisplayPokedexAreaScreen(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), &sPokedexView->screenSwitchState, gAreaSeason, gAreaTimeOfDay, gAreaRank, DEX_SHOW_AREA_SCREEN);
        SetVBlankCallback(gPokedexVBlankCB);
        sPokedexView->screenSwitchState = 0;
        gMain.state = 0;
...
static void Task_ReloadAreaScreen(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        DisplayPokedexAreaScreen(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), &sPokedexView->screenSwitchState, gAreaSeason, gAreaTimeOfDay, gAreaRank, DEX_UPDATE_AREA_SCREEN);
        gMain.state = 0;
        gTasks[taskId].func = Task_WaitForAreaScreenInput;
        break;
    }
        ...
        else
        {
            ...
            for (u32 j = 0; evolutions[i].params != NULL && evolutions[i].params[j].condition != CONDITIONS_END; j++)
            {
                ...
                case IF_NOT_REGION:
                {
                    ...
                    case REGION_GARDEN: StringAppend(gStringVar4, COMPOUND_STRING("The Garden")); break;
                    }
                    break;
                }
                // Gen 8
