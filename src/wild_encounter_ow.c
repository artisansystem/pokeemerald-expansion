...
static bool32 CreateEnemyPartyOWE(struct InfoOWE *info, s32 x, s32 y)
{
    ...
    if (MetatileBehavior_IsWaterWildEncounter(metatileBehavior))
    {
        ...
        seasonTimeRank = GetSeasonTimeRankForEncounters(headerId, wildArea);
        wildMonInfo = gWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].waterMonsInfo;
    }
    else
    {
        wildArea = WILD_AREA_LAND;
        seasonTimeRank = GetSeasonTimeRankForEncounters(headerId, wildArea);
        wildMonInfo = gWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].landMonsInfo;
    }
...
static bool32 CheckCurrentWildMonHeaderForOWE(bool32 shouldSpawnWaterMons)
{
    ...
    if (headerId == HEADER_NONE)
    {
        if (gMapHeader.mapLayoutId == LAYOUT_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS)
        {
            headerId = GetBattlePikeWildMonHeaderId();
            seasonTimeRank = GetSeasonTimeRankForEncounters(headerId, WILD_AREA_LAND);
            return gBattlePikeWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].landMonsInfo != NULL;
        }
        if (gMapHeader.mapLayoutId == LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR)
        {
            headerId = gSaveBlock2Ptr->frontier.curChallengeBattleNum;
            seasonTimeRank = GetSeasonTimeRankForEncounters(headerId, WILD_AREA_LAND);
            return gBattlePyramidWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].landMonsInfo != NULL;
        }
        return FALSE;
    }
    ...
    if (shouldSpawnWaterMons)
    {
        ...
        return gWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].waterMonsInfo != NULL;
    }

    seasonTimeRank = GetSeasonTimeRankForEncounters(headerId, WILD_AREA_LAND);
    return gWildMonHeaders[headerId].encounterTypes[seasonTimeRank.season][seasonTimeRank.timeOfDay][seasonTimeRank.areaRank].landMonsInfo != NULL;
}
