...
static const u16 sGardenSubregionMapsecs[GARDEN_SUBREGION_COUNT][60] = {
    ...
    {
        MAPSEC_HEADMASTERS_HOUSE,
        MAPSEC_KINGSISLE_LOREHOLD,
        MAPSEC_NATURE_PRESERVE,
        MAPSEC_NONE,
    },
...
{
    for (u32 i = GARDEN_SUBREGION_GARDEN; i <= GARDEN_SUBREGION_AUBERON; i++)
    {
        for (u32 j = 0; sGardenSubregionMapsecs[i][j] != MAPSEC_NONE; j++)
        {
            if (mapSecId == sGardenSubregionMapsecs[i][j])
                return i;
        }
    }
    return GARDEN_SUBREGION_GARDEN;
}
